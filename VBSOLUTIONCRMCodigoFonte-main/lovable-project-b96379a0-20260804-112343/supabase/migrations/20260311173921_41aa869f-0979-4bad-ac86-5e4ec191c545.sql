...
CREATE POLICY "Approved can select mensagens_whatsapp" ON public.mensagens_whatsapp FOR SELECT TO authenticated USING ((imobiliaria_id = get_master_user_id()) AND is_approved(auth.uid()));
CREATE POLICY "Approved can insert mensagens_whatsapp" ON public.mensagens_whatsapp FOR INSERT TO authenticated WITH CHECK ((imobiliaria_id = get_master_user_id()) AND is_approved(auth.uid()));
CREATE POLICY "Approved can delete mensagens_whatsapp" ON public.mensagens_whatsapp FOR DELETE TO authenticated USING ((imobiliaria_id = get_master_user_id()) AND is_approved(auth.uid()));
