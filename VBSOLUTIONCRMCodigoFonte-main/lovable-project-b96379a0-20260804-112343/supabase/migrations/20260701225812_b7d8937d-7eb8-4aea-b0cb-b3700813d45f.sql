...
DECLARE
  ...
  v_regra text := 'auto_assign_ludmila_v3_novos_hoje';
BEGIN
  -- Só atua em leads do estágio "novos" cadastrados hoje
  IF LOWER(COALESCE(NEW.estagio, '')) <> 'novos'
     OR COALESCE(NEW.created_at, now())::date <> CURRENT_DATE THEN
    RETURN NEW;
  END IF;
  ...
  IF v_corretor_id IS NOT NULL AND (NEW.corretor_id IS DISTINCT FROM v_corretor_id) THEN
    ...
    VALUES (
      ...
      'Lead do pipeline "Novos Leads" (hoje) direcionado à Ludmila Santos. Regra: ' || v_regra
    ...
    VALUES (
      ...
      'LeadsAutoAssign',
      'auto_assign',
      'Lead novo (hoje) atribuído automaticamente à Ludmila',
      auth.uid(),
      jsonb_build_object(
        'lead_id', NEW.id,
        'lead_nome', NEW.nome,
