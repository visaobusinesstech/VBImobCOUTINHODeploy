 ...
 * Export PDF Consulta CPF — paridade Lovable exportConsultaCPFPDF.
 */

import { normalizeRestrictions, formatCurrencyBRL, formatDateBR } from "./consultaCpfParity";

export async function exportConsultaCPFPDF(result, brandName) {
  ...
  const scoreColor =
    score >= 700 ? [22, 101, 52] : score >= 500 ? [161, 98, 7] : [185, 28, 28];
  doc.setFontSize(22);
  ...
  if (result?.results?.length > 0) {
    ...
    autoTable(doc, {
      ...
      headStyles: { fillColor: [38, 115, 217], textColor: 255 },
      columnStyles: {
        1: { halign: "center" },
        2: { halign: "center" },
      },
  ...
  if (allRestrictions.length > 0) {
    ...
    const restrictionBody = allRestrictions.map((r) => [
      r.descricao,
      r.credor || "—",
      r.valor > 0 ? formatCurrencyBRL(r.valor) : "—",
