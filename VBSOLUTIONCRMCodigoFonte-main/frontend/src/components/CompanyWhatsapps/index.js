...
const WhatsAppModalCompany = ({
  ...
  const handleSubmitConfirmationModal = async () => {
    ...
    if (action === "delete") {
      if (rawId === "" || rawId == null) {
        toast.error("Não foi possível identificar a conexão. Feche o modal e tente novamente.");
      } else {
        try {
          await api.delete(`/whatsapp/${rawId}`);
          toast.success(i18n.t("connections.toasts.deleted"));
          if (typeof removeWhatsAppById === "function") removeWhatsAppById(rawId);
        } catch (err) {
          toastError(err);
        }
      }
