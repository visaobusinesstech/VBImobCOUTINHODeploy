  ...
  useEffect(() => {
    if (loggedUser.id) {
      const socketCompanyId = resolveSocketCompanyId({
        user: { companyId: loggedUser.companyId, id: loggedUser.id },
      });
      ...
      socket.on(`company-${socketCompanyId}-chat`, onCompanyChatPopover);

      return () => {
        socket.off(`company-${socketCompanyId}-chat`, onCompanyChatPopover);
      };
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <div>
      ...
      >
        <Badge color="secondary" variant="dot" invisible={invisible}>
          <ForumRounded style={{ fontSize: 22 }} />
        </Badge>
      </IconButton>
      <Popover
        id={id}
