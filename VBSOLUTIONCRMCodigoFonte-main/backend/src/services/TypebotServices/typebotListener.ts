    ...
    try {
        ...
        if (body.toLocaleLowerCase().trim() !== typebotKeywordFinish.toLocaleLowerCase().trim() && body.toLocaleLowerCase().trim() !== typebotKeywordRestart.toLocaleLowerCase().trim()) {
            ...
            if (messages?.length === 0) {
                const sentMessage = await wbot.sendMessage(resolveReplyJid(msg, ticket.contact), { text: typebotUnknownMessage });
                ...
                for (const message of messages) {
                    if (message.type === 'text') {
                        ...
                        const sentMessage = await wbot.sendMessage(resolveReplyJid(msg, ticket.contact), { text: formatBody(formattedText, ticket) });
                        wbot.store(sentMessage);
                    }
                    ...
                    if (message.type === 'audio') {
                        ...
                        const sentMessage = await wbot.sendMessage(resolveReplyJid(msg, ticket.contact), media);
                        wbot.store(sentMessage);
                    ...
                    //     }
                    //     await wbot.sendMessage(resolveReplyJid(msg, ticket.contact), media);
                    // }
                ...
                if (input) {
                    if (input.type === 'choice input') {
                        ...
                        await delay(typebotDelayMessage)
                        await wbot.sendPresenceUpdate('paused', msg.key.remoteJid)
                        const sentMessage = await wbot.sendMessage(resolveReplyJid(msg, ticket.contact), { text: formattedText });
                        wbot.store(sentMessage);
        ...
        if (body.toLocaleLowerCase().trim() === typebotKeywordRestart.toLocaleLowerCase().trim()) {
            ...
            const sentMessage = await wbot.sendMessage(resolveReplyJid(msg, ticket.contact), { text: typebotRestartMessage })
            wbot.store(sentMessage);
