...
function getDerivedKey(): Buffer {
  const raw =
    process.env.FIGMA_KEY_ENCRYPTION_SECRET ||
  ...
  return Buffer.from(
    crypto
      .createHash("sha256")
      .update(raw || "figma-fallback-secret-change-env")
      .digest()
  );
}

export function encryptFigmaCredential(text: string): string {
...
export function decryptFigmaCredential(payload: string): string | null {
  try {
