...
import { deliverLinkedInAgentReply } from "./deliverLinkedInAgentReply";
import logger from "../../utils/logger";

/** Dispara o agente de IA (prompt da conexão) para mensagens inbound do LinkedIn. */
export async function processLinkedInAgentInbound(params: {
  connection: Whatsapp;
  ticket: Ticket;
  contact: Contact;
  body: string;
  ...
  let prompt: any;
  try {
    const resolved = await resolvePromptWithLlmProvider(
      ticket.companyId,
      Number(linkedPromptId)
    );
  ...
  try {
    ...
    logger.warn(
      `[LINKEDIN-AGENT] memória conversacional ticket=${ticket.id}:`,
      e as any
    );
  }
  ...
  if (
    ...
    const queued = await enqueueAgentInboundJob(agentInboundQueue, {
      ...
      channel: String(connection.channel || ticket.channel || "linkedin").toLowerCase()
    });
    if (queued) {
      logger.info(
        `[LINKEDIN-AGENT] enfileirado ticket=${ticket.id} wid=${messageWid}`
      );
      return;
    }
  }
  ...
  try {
      ...
      try {
        ...
        if (r.handled) {
          ...
          if (safeIntentMessage) {
            await deliverLinkedInAgentReply({
        ...
        logger.warn(`[LINKEDIN-AGENT] intents ticket=${ticket.id}:`, e as any);
      }
    }
    ...
    if (reply) {
      if (await shouldRunSmartActionTriggersForPrompt(prompt, ticket)) {
        try {
          ...
          logger.warn(
            `[LINKEDIN-AGENT] classifier ticket=${ticket.id}:`,
            e as any
          );
        }
      }
    ...
    logger.warn(
      `[LINKEDIN-AGENT] erro ticket=${ticket.id}: ${e?.message || e}`
    );
  }
}
