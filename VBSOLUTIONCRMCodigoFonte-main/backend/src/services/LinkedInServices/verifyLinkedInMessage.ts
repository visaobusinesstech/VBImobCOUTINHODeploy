...
import { SendLinkedInResult } from "./sendLinkedInMessage";
import { emitTelegramTicketSocket } from "../TelegramServices/emitTelegramTicketSocket";
import { markTicketUnderAgentAttendance } from "../TicketServices/markTicketUnderAgentAttendance";

export async function verifyOutboundLinkedInMessage(
  sendResult: SendLinkedInResult,
  ticket: Ticket,
  contact: Contact,
  bodyFallback?: string,
  ...
  const ticketChannel = ticket.channel || "linkedin";
...
export async function verifyInboundLinkedInMessage(
  messageId: string,
  body: string,
  ticket: Ticket,
  contact: Contact
  ...
  const messageData = {
    ...
    channel: ticket.channel || "linkedin",
    mediaType: "conversation"
  };
