  ...
  try {
      ...
      remoteJid ||
        (isGroup
          ...
          : channel === "sms"
            ? `${cleanNumber}@sms`
            : channel === "telegram"
              ? `${cleanNumber}@telegram`
              : channel === "telegram_oficial"
                ? `${cleanNumber}@telegram_user`
                : channel === "linkedin"
                  ? `${cleanNumber}@linkedin`
          ...
          : channel === "telegram"
            ...
            : channel === "telegram_oficial"
              ...
              : channel === "linkedin"
                ...
                : "@s.whatsapp.net";
      logger.info(
        `[RDS-LID-GUARD] remoteJid '${fallbackRemoteJid}' não contém número real '${cleanNumber}', corrigindo para ${cleanNumber}${channelSuffix}`
      );
      fallbackRemoteJid = `${cleanNumber}${channelSuffix}`;
    }
    ...
    let updateImage =
      ((!contact ||
        ...
        (wbot ||
          ["instagram", "facebook", "telegram", "telegram_oficial"].includes(
            channel
          ))) ||
      false;
    ...
    if (contact) {
      ...
      ["facebook", "instagram", "sms", "telegram", "telegram_oficial"].includes(
        channel
      )
    ) {
