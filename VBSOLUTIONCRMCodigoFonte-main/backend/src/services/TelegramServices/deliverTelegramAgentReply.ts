...
import { sendTelegramUserMessage } from "../TelegramUserServices/sendTelegramUserMessage";
import logger from "../../utils/logger";

export async function deliverTelegramAgentReply(params: {
  connection: Whatsapp;
  ticket: Ticket;
  contact: Contact;
  prompt: { id: number };
  ...
  for (const block of blocks) {
    ...
    if (isAgentOutboundGuardEnabled()) {
      ...
      if (!guard.send) {
        logger.info(
          `[TELEGRAM-AGENT] outbound guard (${guard.reason}) ticket=${params.ticket.id}`
        );
        continue;
      }
    }
    ...
    const sendResult =
      params.connection.channel === "telegram_oficial"
