...
import { getAccessTokenFromPage, getSubscribedApps, subscribeApp } from "../services/FacebookServices/graphAPI";
...
async function main() {
  ...
  console.log("BEFORE", await getSubscribedApps(pageId, longLivedPageToken));

  await subscribeApp(pageId, longLivedPageToken);

  console.log("AFTER", await getSubscribedApps(pageId, longLivedPageToken));
}
