...
BEGIN
  ...
  -- Dedup por telefone (comparando os últimos 8 dígitos, cobrindo variações
  -- de DDI/DDD e do 9º dígito). Aceita match a partir de 8 dígitos normalizados.
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    INSERT INTO public.radarzap_leads_auditoria (
      ...
      NEW.id,
      NEW.imobiliaria_id,
      'crm_lead_dedup_vinculado',
      NEW.aprovado_por,
      OLD.status,
      NEW.status,
      jsonb_build_object(
        'crm_lead_id', v_lead_id,
        'motivo', 'telefone_match_suffix8',
        'telefone', v_telefone
      ),
      NEW.revisao_notas
    );
