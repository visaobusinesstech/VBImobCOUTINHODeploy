...
-- Fix ALL RLS policies to be PERMISSIVE across all tables
...
CREATE POLICY "Approved update imoveis" ON public.imoveis FOR UPDATE TO authenticated USING ((imobiliaria_id = get_master_user_id()) AND is_approved(auth.uid()));
