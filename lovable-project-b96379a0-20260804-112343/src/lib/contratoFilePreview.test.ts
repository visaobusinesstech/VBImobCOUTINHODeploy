...
describe("contratoFilePreview", () => {
  ...
  it("extrai nome do arquivo ignorando querystring", () => {
    expect(
      getContratoFileName("https://files.example.com/storage/v1/object/sign/contratos/abc123/doc.docx?token=123"),
    ).toBe("doc.docx");
  });
  ...
  it("reconhece imagem, vídeo e texto em caminhos privados ou URLs legadas", () => {
    expect(getContratoFilePreviewKind("https://files.example.com/storage/v1/object/sign/contratos/abc123/foto.jpeg?token=123")).toBe("image");
