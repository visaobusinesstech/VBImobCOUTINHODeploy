 ...
 * As mesmas chaves são resolvidas no Edge Function `nutricao-processar`.
 */

export type GrupoVariavel = "lead" | "imovel" | "imobiliaria" | "data";

export interface VariavelNutricao {
  chave: string;
  label: string;
  grupo: GrupoVariavel;
  exemplo: string;
...
export const VARIAVEIS_NUTRICAO: VariavelNutricao[] = [
...
export type LeadLike = Record<string, unknown> | null | undefined;
export type ImovelLike = Record<string, unknown> | null | undefined;
export type ConfigLike = Record<string, unknown> | null | undefined;

const txt = (v: unknown) => (v === null || v === undefined ? "" : String(v));

export function montarContextoNutricao(
  lead: LeadLike,
  imovel: ImovelLike,
  config: ConfigLike,
  extras?: { corretor?: string | null; imovelLink?: string | null },
...
export const CONTEXTO_EXEMPLO: Record<string, string> = VARIAVEIS_NUTRICAO.reduce(
  (acc, v) => ({ ...acc, [v.chave]: v.exemplo }),
  {} as Record<string, string>,
);

/** Substitui {{chave}} pelo valor do contexto; chaves vazias somem junto com espaços duplicados. */
export function aplicarVariaveis(texto: string, ctx: Record<string, string>) {
  return texto
    .replace(/\{\{\s*([\w]+)\s*\}\}/g, (_m, key: string) => ctx[key] ?? "")
    .replace(/[ \t]{2,}/g, " ")
    .replace(/ +([,.!?;:])/g, "$1")
    .trimStart();
}

export function variaveisUsadas(texto: string) {
  const encontradas = [...texto.matchAll(/\{\{\s*([\w]+)\s*\}\}/g)].map((m) => m[1]);
  return [...new Set(encontradas)];
}

export function variaveisDesconhecidas(texto: string) {
  return variaveisUsadas(texto).filter((v) => !CHAVES_VALIDAS.has(v));
}

export function variaveisSemValor(texto: string, ctx: Record<string, string>) {
...
export const BLOCOS_MENSAGEM: BlocoMensagem[] = [
  {
    id: "abertura",
