...
const defaultLogoLight = DEFAULT_LOGO_LIGHT;
const defaultLogoDark = DEFAULT_LOGO_DARK;
...
/** Modo escuro: cinzas (shell do CRM). Análises Ads usa fundo próprio. */
const DARK_BG_DEFAULT = "#2d2d2d";
const DARK_BG_PAPER = "#3a3a3a";
const DARK_BG_ELEVATED = "#454545";
/** Cards de KPI / quadros no escuro */
    ...
    return createTheme(
        {
          ...
          overrides: {
            ...
            MuiInputLabel: {
              root: {
                color: mode === "light" ? "rgba(0, 0, 0, 0.6)" : "rgba(255, 255, 255, 0.78)",
                "&.Mui-focused": {
                  color: mode === "light" ? "rgba(0, 0, 0, 0.6)" : "rgba(255, 255, 255, 0.9)",
                },
              },
            },
            MuiFormLabel: {
           ...
           */
          calculatedLogoDark: () => {
            const dark = logosLookSameUrl(appLogoDark, appLogoLight)
              ? defaultLogoDark
              : appLogoDark;
            return dark && String(dark).trim() ? dark : defaultLogoDark;
          },
          /** Logo para fundo claro: só appLogoLight ou default preta */
          calculatedLogoLight: () =>
            appLogoLight && String(appLogoLight).trim()
              ? appLogoLight
              : defaultLogoLight,
        },
        locale
    ...
    (async () => {
      try {
        if (cid != null) {
          ...
          if (!fL) fL = await gp("appLogoLight", cid);
          let fD = pickVal(rows, "appLogoDark");
          if (!fD) fD = await gp("appLogoDark", cid);
          let fF = pickVal(rows, "appLogoFavicon");
          if (!fF) fF = await gp("appLogoFavicon", cid);
          let fT = pickVal(rows, "appLogoTickets");
          if (!fT) fT = await gp("appLogoTickets", cid);
          const name = pickVal(rows, "appName");
          const resolvedName = name != null ? String(name) : (await gp("appName", cid)) || "VBSolution";
        ...
        batchUpdates(() => {
          ...
          setAppLogoLight(lLight ? getBackendUrl() + "/public/" + lLight : defaultLogoLight);
          setAppLogoDark(lDark ? getBackendUrl() + "/public/" + lDark : defaultLogoDark);
          setAppLogoFavicon(lFav ? getBackendUrl() + "/public/" + lFav : defaultLogoFavicon);
          setAppLogoTickets(lTick ? getBackendUrl() + "/public/" + lTick : "");
          setAppName(aName || "VBSolution");
        });
      } catch (e) {
