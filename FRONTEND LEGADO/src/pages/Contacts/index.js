...
import NotionTag from "../../components/ui/NotionTag";
...
const Contacts = () => {
  ...
  const crmImportTick = useCrmImportRefresh("companies");
  ...
  const backendUrl = (api?.defaults?.baseURL) || (getBackendUrl && getBackendUrl());
  ...
  const handleBulkDeleteConfirm = async () => {
    try {
      if (deleteType === 'all') {
        ...
        // Zera imediatamente o estado local
        dispatch({ type: "RESET" });
        ...
        // Garante que a tela reflita o estado real do backend
        // (sem necessidade de o usuário apertar F5 manualmente)
        history.go(0);
      } else {
        ...
        for (let i = 0; i < contactIds.length; i += CHUNK) {
          ...
          await api.post("/contacts/bulk-delete", { contactIds: chunk });
        }
        dispatch({ type: "RESET" });
        setTotalContactsCount((prev) => Math.max(0, prev - contactIds.length));
        setRefreshContactsKey((k) => k + 1);
        toast.success(`${contactIds.length} contato(s) excluído(s) com sucesso`);
      }
    ...
    const delayDebounceFn = setTimeout(() => {
      const fetchContacts = async () => {
        try {
          ...
          setTotalContactsCount(data?.count || 0);
        } catch (err) {
          toastError(err);
          dispatch({ type: "RESET" });
          setHasMore(false);
  ...
  }, [searchParam, pageNumber, selectedTags, refreshContactsKey, crmImportTick]);
  ...
  const selectAllCheckboxStatus = () => {
    ...
    if (selectedInCurrentPage === 0) return { checked: false, indeterminate: false };
    if (selectedInCurrentPage === currentPageIds.length) return { checked: true, indeterminate: false };
  ...
  return (
    <ActivitiesStyleLayout
      ...
      rightFilters={<CrmImportExportButton pageKey="companies" />}
      navActions={
        <>
    ...
    >
      ...
      <ContactModal
        ...
        onSave={(contact) => {
          ...
          setTotalContactsCount((prev) => (contact.id && !contacts.some((c) => c.id === contact.id) ? prev + 1 : prev));
        }}
      />
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center">Listas</TableCell>
              ...
              {contacts.map((contact) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="center">
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "center", maxWidth: 220, margin: "0 auto" }}>
                        {(contact.ticketLists || []).map((list) => (
                          <ContactTag
                            key={`contact-list-${contact.id}-${list.id}`}
                            tag={list}
                          />
                        ))}
                        {!(contact.ticketLists || []).length ? (
                          <span style={{ opacity: 0.45 }}>—</span>
                        ) : null}
                      </div>
                    </TableCell>
                    <TableCell align="center">
                      {contact.pipelineStage?.label ? (
                        <NotionTag
                          fullLabel
                          label={contact.pipelineStage.label}
                          color={contact.pipelineStage.color || "#64748b"}
                          title={`Etapa: ${contact.pipelineStage.label}`}
                        />
                      ) : (
                        <span style={{ opacity: 0.45 }}>—</span>
                      )}
                    ...
                    <TableCell align="center">
                      <CrmSourceBadge crmSource={contact.crmSource} showLabel />
                    </TableCell>
                    <TableCell align="center">
