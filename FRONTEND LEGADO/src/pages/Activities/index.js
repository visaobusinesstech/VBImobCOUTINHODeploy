...
const COMPLETED_ACTIVITY_STATUSES = [
  "completed",
  "concluido",
  "concluído",
  "concluida",
...
const isActivityCompleted = (status) =>
  COMPLETED_ACTIVITY_STATUSES.includes(String(status || "").toLowerCase().trim());
          ...
          {activities.length > 0 ? (
            activities.map((activity) => (
              <TableRow key={activity.id}>
                ...
                <TableCell>
                  <CrmSourceBadge crmSource={activity.crmSource} showLabel />
                </TableCell>
...
const ActivitiesCalendar = ({ activities, onCreate }) => {
  ...
  const eventPropGetter = (event) => {
    ...
    if (isActivityCompleted(status)) style = { backgroundColor: "#D1FAE5", border: "1px solid #A7F3D0", color: "#065F46", borderRadius: 10, padding: "6px 8px", fontSize: 12 };
  ...
  const conclu = activities.filter(a => isActivityCompleted(a.status)).length;
  return (
    <div
...
const Activities = () => {
  const crmImportTick = useCrmImportRefresh("activities");
  const theme = useTheme();
  ...
  const headerStats = useMemo(() => {
    ...
    const completed = base.filter(a => isActivityCompleted(a.status)).length;
    return [
  ...
  const actionsRight = (
    <>
      <CrmImportExportButton pageKey="activities" />
      ...
      {loading ? (
        ...
        <>
          {viewMode === "dashboard" && (() => {
            ...
            const completed = filteredActivities.filter(a => isActivityCompleted(a.status)).length;
            ...
            const overdue = filteredActivities.filter(a => {
              ...
              if (isActivityCompleted(a.status)) return false;
            ...
            filteredActivities.forEach(a => {
              ...
              if (isActivityCompleted(st)) completedPerDay[k] = (completedPerDay[k] || 0) + 1;
              if (st === "in_progress") inProgPerDay[k] = (inProgPerDay[k] || 0) + 1;
              const d = new Date(a.date || Date.now());
              if (d < todayMid && !isActivityCompleted(st)) overduePerDay[k] = (overduePerDay[k] || 0) + 1;
            });
            ...
            filteredActivities.forEach(a => {
              ...
              const st = String(a.status || "").toLowerCase();
              if (isActivityCompleted(st)) ownerMap[name].completed += 1;
              if (st === "in_progress") ownerMap[name].inProgress += 1;
              if (a.date) {
                const d = new Date(a.date);
                if (d < todayMid && !isActivityCompleted(st)) ownerMap[name].overdue += 1;
              }
            });
            ...
            <div ref={kanbanRef} style={{ height: '100%', width: '100%' }}>
              <KanbanBoard
                ...
                onMove={async (activityId, sourceCol, destCol) => {
                  ...
                  try {
                    const updated = await activitiesService.update(id, { status: newStatus });
                    if (updated?.status) {
                      setActivitiesState(prev =>
                        prev.map(a => Number(a.id) === id ? { ...a, ...updated } : a)
                      );
                    }
                  } catch (err) {
                    setActivitiesState(prev =>
                      prev.map(a => Number(a.id) === id ? { ...a, status: previousStatus } : a)
                    );
                    toastError(err);
                  }
                }}
