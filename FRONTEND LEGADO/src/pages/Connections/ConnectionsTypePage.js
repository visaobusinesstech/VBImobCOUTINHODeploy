...
const ConnectionsTypePage = () => {
  ...
  const handleMetaChannelConnected = async () => {
    if (typeof fetchWhatsApps === "function") {
      await fetchWhatsApps({ silent: true });
      // Socket/listagem podem atrasar um pouco após OAuth no popup.
      setTimeout(() => {
        try {
  ...
  const handleRepairMetaMessenger = async (whatsAppId) => {
    try {
      toast.info("Reparando webhooks Instagram/Facebook…", { autoClose: 2500 });
      const { data } = await api.post(
        `/whatsapp/${whatsAppId}/repair-meta-messenger`
      );
      const parts = [];
      if (data?.appWebhooks?.ok) parts.push("app OK");
      else parts.push("app falhou");
      if (data?.pageSubscribe?.ok) parts.push("Página OK");
      else if (data?.pageSubscribe) parts.push("Página falhou");
      if (data?.igSubscribe?.ok) parts.push("IG OK");
      else if (data?.igSubscribe) parts.push("IG falhou");
      if (data?.hasIgObject === false) {
        parts.push("objeto Instagram ausente no app");
      }
      if (data?.dmSync?.imported > 0) {
        parts.push(`${data.dmSync.imported} DM(s) importada(s)`);
      }

      if (data?.needsInstagramLoginReconnect) {
        toast.warning(
          "Esta conexão Instagram foi feita via Página/Facebook Login e NÃO recebe DMs de clientes com suas permissões aprovadas (instagram_business_*). Apague a conexão e reconecte com o botão «Conectar Instagram Login».",
          { autoClose: 18000 }
        );
      } else if (data?.ok) {
        toast.success(
          `Webhooks Meta reparados (${parts.join(
            ", "
          )}). Envie um DM de teste agora.`,
          { autoClose: 9000 }
        );
      } else {
        toast.warning(
          `Reparo parcial (${parts.join(", ")}). ${
            Array.isArray(data?.nextSteps) && data.nextSteps[0]
              ? data.nextSteps[0]
              : "Confira Alert Inbox e Advanced Access no Meta Developers."
          }`,
          { autoClose: 14000 }
        );
      }
      ...
      toast.error(
        err?.response?.data?.error ||
          err?.response?.data?.message ||
          "Não foi possível reparar webhooks do Instagram/Facebook."
      );
    }
  };
      ...
      <div className={`${classes.sessionCell} ${classes.actionGroup}`}>
        ...
        {((whatsApp.channel === "facebook" || whatsApp.channel === "instagram") && whatsApp.status === "CONNECTED") && (
          <>
            ...
            <Tooltip title="Reinscrever webhooks Meta (DMs Instagram/Messenger)">
              <ConnectionMinimalButton
                onClick={() => handleRepairMetaMessenger(whatsApp.id)}
              >
                Reparar DMs
              </ConnectionMinimalButton>
            </Tooltip>
    ...
    <>
      ...
      >
        ...
        <ConnectionsManageToolbar
          ...
          extraActions={
            integration?.key === "meta-ads" ? (
              <MetaAdsOAuthAccounts variant="toolbar" />
            ) : null
          }
        />
        ...
        <>
        {(integration?.key === "facebook" || integration?.key === "instagram") &&
            ...
            <MetaMessengerOAuthConnect
              ...
              disabled={
                integration.key === "facebook"
                  ? planConfig?.plan && !planConfig.plan.useFacebook
                  : planConfig?.plan && !planConfig.plan.useInstagram
              }
              onSuccess={handleMetaChannelConnected}
            />
          </Box>
        ) : null}
