...
const BRAND_RENDERERS = {
  ...
  "meta-ads": ({ size, color }) => (
    <MetaAdsBrandIcon size={size} color={color || "#0081FB"} />
  ),
  tiktok: ({ size }) => <BrandImageIcon src={tiktokIcon} size={size} alt="TikTok" />,
  linkedin: ({ size, color }) => (
  ...
  hubspot: ({ size }) => <BrandImageIcon src={hubspotIcon} size={size} alt="HubSpot" />,
  clickup: ({ size }) => <BrandImageIcon src={clickupIcon} size={size} alt="ClickUp" />,
  pipedrive: ({ size }) => <BrandImageIcon src={pipedriveIcon} size={size} alt="Pipedrive" />,
  hotmart: ({ size }) => <BrandImageIcon src={hotmartIcon} size={size} alt="Hotmart" />,
  cakto: ({ size }) => <BrandImageIcon src={caktoIcon} size={size} alt="Cakto" />,
  "vbsolution-api": ({ size }) => (
...
/** Marcas pretas/quase pretas — invisíveis em cards escuros se não inverter. */
...
export function resolveBrandIconColor(brandKey, theme, { accentColor, background, plain }) {
  ...
  if (brandKey === "github") {
    if (isDark) {
      return isNearBlackHex(accentColor) ? "#f4f4f5" : accentColor;
    }
  ...
  if (brandKey === "notion") {
    return accentColor && !isNearBlackHex(accentColor)
      ? accentColor
      : isDark
        ? "#f4f4f5"
        : "#000000";
  }
  if (brandKey === "meta-ads") {
    return accentColor || "#0081FB";
  }
  if (plain) {
    if (isDark && isNearBlackHex(accentColor || background)) {
      return "#f4f4f5";
    }
...
export function getBrandVisual(integration) {
  const map = {
    ...
    "meta-ads": { brandKey: "meta-ads", accent: "#0081FB", iconBg: "#0866FF", plain: true },
    "linkedin-messaging": {
      brandKey: "linkedin",
      accent: "#0A66C2",
      iconBg: "#0A66C2",
    ...
    hotmart: { brandKey: "hotmart", accent: "#F04E23", iconBg: "#F04E23" },
    cakto: { brandKey: "cakto", accent: "#0B8F5A", iconBg: "#0B8F5A" },
    tiktok: { brandKey: "tiktok", accent: "#111111", iconBg: "#111111" },
    "vbsolution-api": {
