...
import AgentScriptEditor, { getHashMediaFilter, getStarFilter } from "./AgentScriptEditor";
...
export default function AgentEditorPage() {
  ...
  /** `null` | `{ type: 'media' | 'vars', filter: string }` — modal Apple para # mídias e * variáveis */
  ...
  const extractKnowledgeAttachment = async (file) => {
    ...
    try {
      ...
      // Mantém o binário no storage para o roteiro #slug enviar via WhatsApp.
      let fileUrl = "";
      try {
        const up = new FormData();
        up.append("file", file);
        up.append("kind", inferUploadKind(file));
        const { data: uploaded } = await api.post("/prompt/attendance-flow/upload", up, {
          headers: { "Content-Type": "multipart/form-data" },
          timeout: 0,
          maxContentLength: Infinity,
          maxBodyLength: Infinity
        });
        fileUrl = String(uploaded?.url || "").trim();
      } catch (upErr) {
        console.warn("[extractKnowledge] upload binário falhou", upErr);
      }

      setV2((prev) => {
        const prevSources = Array.isArray(prev.knowledge?.sources) ? prev.knowledge.sources : [];
        const prevLibrary = Array.isArray(prev.mediaLibrary) ? prev.mediaLibrary : [];
        const slugTaken = prevLibrary.some(
          (m) => normalizeMediaSlug(String(m.slug || "")) === suggestedSlug
        );
        const nextLibrary =
          fileUrl && !slugTaken
            ? [
                ...
                {
                  slug: suggestedSlug,
                  name: name.replace(/\.[^.]+$/, "").slice(0, 120) || suggestedSlug,
                  fileUrl,
                  fileType: inferMediaFileTypeLabel(file),
                  caption: ""
                }
              ]
            : prevLibrary;
          ...
          knowledge: {
            ...
            sources: [
              ...prevSources,
              {
                ...
                metadata: { kind: "extracted_upload", sendable: Boolean(fileUrl) }
              }
            ]
          }
        };
      ...
      toast.success(
        fileUrl
          ? `Anexo na base e pronto para #${suggestedSlug} no roteiro. Salve o agente.`
          : "Anexo processado na base (texto). Para enviar no WhatsApp, adicione em Mídias."
      );
    } catch (e) {
      toastError(e);
    }
  ...
  const syncScriptPickerFromCursor = useCallback((val, pos) => {
    const hashMedia = getHashMediaFilter(val, pos);
    const star = getStarFilter(val, pos);
    if (hashMedia === null && star === null) {
      scriptPickerDismissedRef.current = false;
      setScriptMenu(null);
      return;
    }
    ...
    if (hashMedia !== null) setScriptMenu({ type: "media", filter: hashMedia });
  ...
  const scriptPickerItems = useMemo(() => {
    ...
    const rows = [];
    for (const m of v2.mediaLibrary || []) {
      const slug = String(m.slug || "").replace(/^[#/]/, "");
      const slugKey = normalizeMediaSlug(slug);
  ...
  const addMedia = () => {
    ...
    const nextSlug = normalizeMediaSlug(mediaDraft.slug);
    const prevLibrary = Array.isArray(v2.mediaLibrary) ? v2.mediaLibrary : [];
    if (
      prevLibrary.some(
        (m) => normalizeMediaSlug(String(m.slug || "")) === nextSlug
      )
    ) {
      toast.error("Já existe uma mídia com este slug.");
      return;
    }
    setV2((prev) => ({
      ...prev,
      mediaLibrary: [
        ...(Array.isArray(prev.mediaLibrary) ? prev.mediaLibrary : []),
        {
          slug: nextSlug,
        ...
        <Box className={classes.pageRoot}>
          ...
          <Box minWidth={0}>
              ...
              <>
                ...
                >
                  <AgentScriptEditor
                    ...
                    placeholder="Roteiro: quebra simples = mesma bolha; linha em branco = bolha nova; — sozinho = mesma bolha com quebra visual (some no WhatsApp). #mídia e *variáveis. / é texto normal."
                ...
                <AgentScriptPickerModal
                  ...
                  hint={
                    scriptMenu?.type === "vars"
                      ...
                      : "Mídias do agente. Filtre digitando após # (ex.: #catalogo)."
                  }
                  onClose={dismissScriptPicker}
                  onPick={(it) => insertScriptToken(it.insert)}
                />
                <Typography variant="caption" color="textSecondary" style={{ marginTop: 8, display: "block" }}>
                  <strong>?</strong> no canto, <strong>F1</strong> ou <strong>Ctrl+?</strong>: guia. <strong>#</strong> (mídias) e <strong>*</strong> (variáveis) abrem o seletor. A <strong>/</strong> no roteiro é texto puro. Quebra simples = mesma bolha; linha em branco = bolha nova; hífen <strong>-</strong> sozinho = mesma bolha com parágrafo; travessão <strong>—</strong> sozinho = mesma bolha com quebra visual. Nova etapa: <strong>---</strong> ou <strong># ETAPA</strong>. Aba <strong>Contexto</strong> (direita): quando usar o roteiro. Aba <strong>Ações</strong>: automações. Salve ao terminar.
                </Typography>
            ...
            {tab === TAB_INVENTORY && (
              <Box>
                <AgentInventoryTab
                  inventory={v2.inventory || { enabled: true, productIds: [] }}
                  onChange={(inventory) => updateV2({ inventory })}
          ...
          <Box>
            ...
            {tab === TAB_SCRIPT && (
              <Paper className={classes.settingsCard} elevation={0} style={{ marginTop: 12 }}>
                <Typography className={classes.cardTitle}>Contexto do roteiro</Typography>
                <Typography variant="caption" color="textSecondary" display="block" style={{ marginBottom: 8 }}>
                  Explique ao agente o objetivo deste roteiro, quando ele deve seguir as etapas e quando deve
                  pausar para entender o pedido (ex.: se o cliente só diz “quero ajuda”, pergunte o que precisa
                  antes de disparar a etapa 1).
                </Typography>
                <TextField
                  fullWidth
                  multiline
                  ...
                  placeholder={
                    "Ex.: Este roteiro qualifica leads comerciais do VBSolution. Use as etapas só quando o contato " +
                    "já mostrou interesse em conhecer/comprar o CRM. Se a pessoa pedir ajuda genérica, pergunte " +
                    "primeiro o que ela precisa — não despeje a etapa 1 automaticamente."
                  }
                  value={v2.attendance.settings.scriptContext || ""}
                  onChange={(e) => updateAttendanceSettings({ scriptContext: e.target.value })}
                  margin="dense"
                  variant="outlined"
                  size="small"
                  className={classes.notionField}
            ...
            {tab === TAB_RULES && (
              <Paper className={clsx(classes.settingsCard, classes.magicPanel)} elevation={0}>
                ...
                <TextField
                  fullWidth
                  label="Delay de Resposta (s)"
                  type="number"
                  value={v2.agent.responseDelay}
                  onChange={(e) => {
                    const n = Number(e.target.value);
                    updateAgent({
                      responseDelay: Number.isFinite(n) ? Math.max(0, Math.min(120, Math.round(n))) : 0
                    });
                  }}
                  inputProps={{ min: 0, max: 120, step: 1 }}
                  helperText="Após gerar a resposta, a IA aguarda este tempo e exibe “digitando…” no WhatsApp antes de enviar (mais humano)."
                  margin="dense"
                  variant="outlined"
                  size="small"
                  className={clsx(classes.notionField, classes.dialogFormField)}
                  style={{ marginTop: 4 }}
                />
                <Box display="flex" flexWrap="wrap" style={{ gap: 6, marginTop: 4, marginBottom: 4 }}>
                  {[0, 5, 10, 15, 20, 30, 60].map((sec) => (
                    <Button
                      key={sec}
                      size="small"
                      variant={Number(v2.agent.responseDelay) === sec ? "contained" : "outlined"}
                      color="primary"
                      onClick={() => updateAgent({ responseDelay: sec })}
                      style={{ minWidth: 44, textTransform: "none" }}
                    >
                      {sec === 0 ? "Off" : `${sec}s`}
                    </Button>
                  ))}
