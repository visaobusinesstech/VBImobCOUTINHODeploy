...
const EmailDashboard = ({ fetchTotals, fetchSeries }) => {
  ...
  const tickColor = isDark ? "#e4e4e7" : "#475569";
  const chartOptions = React.useMemo(
    () => ({
      ...
      scales: {
        y: {
          beginAtZero: true,
          grid: { borderDash: [4, 4], color: isDark ? "rgba(255,255,255,0.14)" : "#E6F0FF" },
      ...
      <div className={classes.content}>
        {smtpChecked && !smtpOk && (
          <Paper
            style={{
              padding: 12,
              border: isDarkPage ? "1px solid rgba(248,113,113,0.45)" : "1px solid #FCA5A5",
              background: isDarkPage ? "rgba(248,113,113,0.14)" : "#FEF2F2",
              color: isDarkPage ? "#fecaca" : "#7F1D1D",
              borderRadius: 8,
              marginBottom: 12,
            }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              <Typography variant="body2" style={{ fontWeight: 600, color: "inherit" }}>
