...
import {
  hasAdsAttribution,
  resolveAttribution
} from "../../components/MetaAdsAttributionPanel";
import {
  resolveWonStageKey,
  getWonStageLabel,
...
const useStyles = makeStyles((theme) => ({
  ...
  cardMetaAdsBadge: {
    ...
    background: "#0081FB",
    color: "#fff",
    boxShadow: "0 1px 4px rgba(0,129,251,0.35)",
    pointerEvents: "none",
  },
  cardMetaAdsBadgeIg: {
  ...
  listTagWrap: {
    ...
    minWidth: 0,
    maxWidth: "100%",
    // tags Notion podem crescer; quebra linha em vez de sobrepor
    "& > *": {
      flex: "0 1 auto",
      maxWidth: "100%",
      minWidth: 0,
...
const LeadsKanbanBoard = ({ columns, leads, onEdit, onAdd, onMove, onDelete, onComplete, contacts, onOpenTagCreator, onOpenPipelineConfig }) => {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      // Desktop: divide pelo nº real de etapas (padrão = 5, como o /5 antigo).
      // Mobile: min 260 + scroll (inalterado).
      const widthPerCol = calcKanbanColumnWidth(el, n, { fallbackPadding: 12 });
      setColPx(widthPerCol);
      setNeedsPan(kanbanNeedsHorizontalPan(el, n, widthPerCol, { fallbackPadding: 12 }));
    };
    calc();
  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd}>
      <div
        ...
        style={{
          gridTemplateColumns: colPx ? `repeat(${Math.max(1, cols.length)}, ${colPx}px)` : undefined,
                        ...
                        {(providedDraggable) => (
                          ...
                          >
                            ...
                            {(() => {
                              ...
                              const listTags = l.ticketLists || l.contact?.ticketLists || [];
                              return (
                                <>
                                  ...
                                  {(() => {
                                    ...
                                    const isIg =
                                      String(adsAttr.platform || "").toLowerCase() ===
                                      "instagram_ads";
                                    return (
                                      <div
                                        className={`${classes.cardMetaAdsBadge} ${
                                          isIg ? classes.cardMetaAdsBadgeIg : ""
                                        }`}
                                        title={
                                          adsAttr.headline ||
                                          adsAttr.campaign ||
                                          (isIg ? "Instagram Ads" : "Meta Ads")
                                        }
                                        style={
                                      ...
                                      >
                                        {isIg ? (
                                          ...
                                          <MetaAdsBrandIcon size={12} color="#fff" />
                                        )}
                                      </div>
                                    );
                                  })()}
                                  {l.crmSource?.provider && (
                                    <div className={classes.cardCrmBadge}>
                                      <CrmSourceBadge crmSource={l.crmSource} variant="card" opacity={0.45} />
                                    </div>
                                  )}
                                  ...
                                  <div className={classes.cardBodyLeft}>
                                    {(l.phone || contact?.number) && (
                                      <div className={classes.cardRow}>
                                        <PhoneIcon style={{ fontSize: 10, color: isDark ? "#e5e7eb" : "#9CA3AF", flexShrink: 0 }} />
                                    ...
                                    >
                                      <div className={classes.listTagWrap}>
                                        {listTags.map((list) => (
                                          <ContactTag
                                            key={`lead-list-${l.id}-${list.id}`}
                                            tag={list}
                                          />
                                        ))}
                                      ...
                                      <AddIcon
                                        className={classes.cardTagAddBtn}
                                        style={{ fontSize: 12, color: isDark ? "#60a5fa" : "#3B82F6", opacity: 0.9 }}
                                      />
                                    </div>
                                    {l.responsible?.name && (
                                      <div className={classes.cardResponsible}>
                                        <PersonOutlineIcon style={{ fontSize: 9, color: isDark ? "#d1d5db" : "#4B5563", flexShrink: 0 }} />
                                  ...
                                  <div className={classes.cardFooter}>
                                    <div className={classes.cardFooterLeft}>
                                      ...
                                      <div
                                        className={classes.cardTimeBadge}
                                        style={{ color: isDark ? "#d1d5db" : "#6B7280" }}
                                      >
                                        <QueryBuilderIcon style={{ fontSize: 12, color: isDark ? "#e5e7eb" : "#9CA3AF" }} />
              ...
              <TableRow key={item.id}>
                ...
                <TableCell>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {(item.ticketLists || item.contact?.ticketLists || []).map((list) => (
                      <ContactTag
                        key={`lead-row-list-${item.id}-${list.id}`}
                        tag={list}
                      />
                    ))}
                    {!(item.ticketLists || item.contact?.ticketLists || []).length
                      ? "—"
                      : null}
                  </div>
                </TableCell>
                <TableCell>
                  {(() => {
                    const stage = item.pipelineStage || item.contact?.pipelineStage;
                    const meta = stage?.label
                      ? { label: stage.label, color: stage.color || "#64748b" }
...
const LeadsSales = () => {
  const crmImportTick = useCrmImportRefresh("leads");
  const classes = useStyles();
  const theme = useTheme();
  const isDark =
  ...
  const [anchorCreatedAt, setAnchorCreatedAt] = useState(null);
  const [anchorClosedAt, setAnchorClosedAt] = useState(null);
  ...
  const navActions = (
    <>
      <CrmImportExportButton pageKey="leads" />
      <IconButton
  ...
  const rightFilters = ({ classes: layout }) => (
    <>
      ...
      <div className={layout.filterItem} onClick={(e) => setAnchorCreatedAt(e.currentTarget)}>
        <CalendarIcon className={layout.calendarIcon} style={{ fontSize: 11 }} />
        <Typography className={layout.filterLabel}>
          {createdAtStart && createdAtEnd
            ? `${createdAtStart.slice(8,10)}/${createdAtStart.slice(5,7)} – ${createdAtEnd.slice(8,10)}/${createdAtEnd.slice(5,7)}`
            : "Data Criação"}
        </Typography>
        <ExpandMoreIcon className={layout.chevronIcon} style={{ fontSize: 11 }} />
      </div>
      <Popover
        open={Boolean(anchorCreatedAt)}
        anchorEl={anchorCreatedAt}
        onClose={() => setAnchorCreatedAt(null)}
        anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
        PaperProps={{ style: { borderRadius: 6, boxShadow: '0 4px 24px rgba(0,0,0,0.12)', border: 'none', minWidth: 240 } }}
      >
        <div style={{ padding: 12 }}>
          <div style={{ display: 'flex', gap: 4, marginBottom: 10, flexWrap: 'wrap' }}>
            {[
              ...
              const sv = start.toISOString().slice(0,10);
              const ev = end.toISOString().slice(0,10);
              const active = createdAtStart === sv && createdAtEnd === ev;
              return (
                <div key={p.label} onClick={() => { setCreatedAtStart(sv); setCreatedAtEnd(ev); setAnchorCreatedAt(null); }}
                  style={{
                    padding: '4px 10px', borderRadius: 4, fontSize: 11, fontWeight: 500,
                    cursor: 'pointer', transition: 'all 0.15s',
                    backgroundColor: active ? '#3b82f6' : (isDark ? 'rgba(255,255,255,0.06)' : '#f3f4f6'),
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, gap: 6 }}>
            <div onClick={() => { setCreatedAtStart(""); setCreatedAtEnd(""); setAnchorCreatedAt(null); }}
              style={{ padding: '4px 10px', fontSize: 11, fontWeight: 500, cursor: 'pointer', borderRadius: 4, color: isDark ? '#9ca3af' : '#6b7280' }}
            >Limpar</div>
            <div onClick={() => setAnchorCreatedAt(null)}
              style={{ padding: '4px 14px', fontSize: 11, fontWeight: 600, cursor: 'pointer', borderRadius: 4, backgroundColor: '#3b82f6', color: '#fff' }}
            >Aplicar</div>
          </div>
        </div>
      ...
      <div className={layout.filterItem} onClick={(e) => setAnchorClosedAt(e.currentTarget)}>
        <CalendarIcon className={layout.calendarIcon} style={{ fontSize: 11 }} />
        <Typography className={layout.filterLabel}>
          {closedAtStart && closedAtEnd
            ? `${closedAtStart.slice(8,10)}/${closedAtStart.slice(5,7)} – ${closedAtEnd.slice(8,10)}/${closedAtEnd.slice(5,7)}`
            : "Data Fechamento"}
        </Typography>
        <ExpandMoreIcon className={layout.chevronIcon} style={{ fontSize: 11 }} />
      </div>
      <Popover
        open={Boolean(anchorClosedAt)}
        anchorEl={anchorClosedAt}
        onClose={() => setAnchorClosedAt(null)}
        anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
        PaperProps={{ style: { borderRadius: 6, boxShadow: '0 4px 24px rgba(0,0,0,0.12)', border: 'none', minWidth: 240 } }}
      >
        <div style={{ padding: 12 }}>
          <div style={{ display: 'flex', gap: 4, marginBottom: 10, flexWrap: 'wrap' }}>
            {[
              ...
              const sv = start.toISOString().slice(0,10);
              const ev = end.toISOString().slice(0,10);
              const active = closedAtStart === sv && closedAtEnd === ev;
              return (
                <div key={p.label} onClick={() => { setClosedAtStart(sv); setClosedAtEnd(ev); setAnchorClosedAt(null); }}
                  style={{
                    padding: '4px 10px', borderRadius: 4, fontSize: 11, fontWeight: 500,
                    cursor: 'pointer', transition: 'all 0.15s',
                    backgroundColor: active ? '#3b82f6' : (isDark ? 'rgba(255,255,255,0.06)' : '#f3f4f6'),
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <input type="date" value={closedAtStart} onChange={(e) => setClosedAtStart(e.target.value)}
              style={{ flex: 1, padding: '5px 6px', fontSize: 11, borderRadius: 4, border: `1px solid ${isDark ? 'rgba(255,255,255,0.15)' : '#d1d5db'}`, background: isDark ? 'rgba(255,255,255,0.06)' : '#fff', color: isDark ? '#e5e7eb' : '#111', outline: 'none' }}
            />
            <span style={{ fontSize: 10, color: isDark ? '#9ca3af' : '#6b7280' }}>–</span>
            <input type="date" value={closedAtEnd} onChange={(e) => setClosedAtEnd(e.target.value)}
              style={{ flex: 1, padding: '5px 6px', fontSize: 11, borderRadius: 4, border: `1px solid ${isDark ? 'rgba(255,255,255,0.15)' : '#d1d5db'}`, background: isDark ? 'rgba(255,255,255,0.06)' : '#fff', color: isDark ? '#e5e7eb' : '#111', outline: 'none' }}
            />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, gap: 6 }}>
            <div onClick={() => { setClosedAtStart(""); setClosedAtEnd(""); setAnchorClosedAt(null); }}
              style={{ padding: '4px 10px', fontSize: 11, fontWeight: 500, cursor: 'pointer', borderRadius: 4, color: isDark ? '#9ca3af' : '#6b7280' }}
            >Limpar</div>
            <div onClick={() => setAnchorClosedAt(null)}
              style={{ padding: '4px 14px', fontSize: 11, fontWeight: 600, cursor: 'pointer', borderRadius: 4, backgroundColor: '#3b82f6', color: '#fff' }}
            >Aplicar</div>
          </div>
        </div>
