...
const Dashboard = () => {
  ...
  const normalizeActivityStatus = (status) => {
    ...
    if (["pendente", "pending", "a fazer", "a_fazer", "todo", "to_do"].includes(s)) return "pending";
    if (["em progresso", "in_progress", "em_progresso", "concluindo", "concluding"].includes(s)) return "in_progress";
    if (["concluído", "concluido", "concluida", "concluída", "completed", "done", "finalizado", "finalizada"].includes(s)) {
      return "completed";
    }
    return s;
  };
  const isActivityCompleted = (status) => normalizeActivityStatus(status) === "completed";
  const isActivityPending = (status) => normalizeActivityStatus(status) === "pending";

  const pendingActivities = useMemo(() => {
    return activities
      .filter((a) => {
        const statusOk = isActivityPending(a.status);
            ...
            items.map((e, idx) => {
              ...
              return (
                <div
                  ...
                  data-agenda-event
                  style={{ backgroundColor: bg, colorScheme: "light" }}
                  onClick={() => history.push("/schedules")}
                >
                  <span className={classes.dot} style={{ backgroundColor: color }} />
                  <Typography className={`${classes.blockItemMeta} ${classes.agendaTime}`} component="span">
                    {e.time ? moment(e.time).format("HH:mm") : "--:--"}
                  </Typography>
                  <Typography className={classes.blockItemTitle} style={{ flex: 1 }}>
                    {e.title}
                  </Typography>
                </div>
              );
      ...
      return (
        <ul className={classes.blockList}>
          {recentActivities.map((a, idx) => {
            const normalized = normalizeActivityStatus(a.status);
            const completed = normalized === "completed";
            let color = "#3b82f6"; // em andamento
            if (completed) color = "#10b981";
            else if (normalized === "pending") color = "#f59e0b";
            else if (normalized === "backlog") color = "#6b7280";
            const hasDate = !!a.date;
            const d = hasDate ? moment(a.date) : null;
            const isFuture = d ? d.isAfter(moment()) : false;
            const isOverdue = !completed && hasDate ? (!isFuture && !d.isSame(moment(), "day")) : false;
            const when = d ? d.fromNow(true) : "-";
            const metaText = completed
              ? (hasDate ? `Concluída · ${d.format("DD/MM/YYYY")}` : "Concluída")
              : hasDate
                ? (isFuture
                    ? `Atividade a vencer em ${when}`
                    : d.isSame(moment(), "day")
  ...
  return (
    <>
      {user.profile === "user" && user.showDashboard === "disabled" ? (
        ...
        <MainContainer className={classes.mainContainer} autoHeight>
          <Paper
              ...
              <div className={classes.miniTopbar}>
                ...
                <div className={classes.miniTopbarRight}>
                  ...
                  <IconButton
                    ...
                    title="Mais"
                  >
                    <MoreHorizIcon style={{ fontSize: 16, color: theme.palette.text.primary }} />
                  </IconButton>
                </div>
              </div>
