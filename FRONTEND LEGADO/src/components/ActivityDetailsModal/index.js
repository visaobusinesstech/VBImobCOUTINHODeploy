...
const statusOptions = [
  ...
  { value: "completed", label: "CONCLUÍDA", color: "#10b981", bg: "#d1fae5", darkBg: "rgba(16,185,129,0.15)" },
];
...
const activityToForm = (activity) => ({
  ...
  status: (() => {
    const s = String(activity?.status || "pending").toLowerCase();
    if (["done", "concluido", "concluído", "concluida", "concluída", "finalizado", "finalizada"].includes(s)) {
      return "completed";
    }
