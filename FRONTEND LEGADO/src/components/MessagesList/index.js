...
const useStyles = makeStyles((theme) => ({
  ...
  messagesListWrapperFill: {
    flex: "1 1 0%",
    minHeight: 0,
    height: 0, // força o filho a respeitar o flex e rolar internamente
    minWidth: 0,
    display: "flex",
    flexDirection: "column",
    overflow: "hidden",
  ...
  messagesList: {
    ...
    WebkitOverflowScrolling: "touch",
    overscrollBehavior: "contain",
    // Evita que o flex “estique” a lista e force scroll na página
    minHeight: 0,
    ...theme.scrollbarStyles,
  },
  /** Empurra as bolhas para o rodapé quando há poucas mensagens (chat “fixo”). */
  messagesListBottomSpacer: {
    display: "none",
  },
  messagesListJustifyEnd: {
...
const MessagesList = ({
  ...
  const isOutgoingAgentBubble = (message) => {
    ...
    const isMetaChannel =
      ticketCh === "facebook" || ticketCh === "instagram";
    ...
    // Meta: eco da Graph às vezes grava fromMe sem fromAgent; se o bot está
    // atendendo e não há humano, trata como agente (azul), igual WhatsApp/Telegram.
    if (isMetaChannel && !asBool(message.isPrivate)) {
      const noHumanAttendant =
        ticketUserId == null || Number(ticketUserId) === 0;
      if (
        noHumanAttendant &&
        (asBool(ticketIsBot) || asBool(ticketUseIntegration))
      ) {
        return true;
      }
    }
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      const fetchMessages = async () => {
        ...
        // Prefere id numérico (estável); uuid da URL como fallback
        const apiTicketKey =
        ...
        try {
          const { data } = await api.get("/messages/" + apiTicketKey, {
            params: { pageNumber },
          });

          if (cancelled) return;

          // Aceita resposta se ainda estamos no mesmo ticket da URL
          // (não descartar por race de ref — isso deixava loading eterno e chat em branco)
          if (String(currentTicketId.current) !== String(ticketId)) {
            return;
          }
          ...
          currentTicketNumericId.current =
            ...
            (msgs.length > 0 ? msgs[0].ticketId : currentTicketNumericId.current);

          if (pageNumber === 1 && msgs.length > 0) {
            scrollToBottom();
          }
        } catch (err) {
          if (!cancelled) {
  ...
  }, [pageNumber, ticketId, ticketInternalId, history]);

  // Garante bolhas visíveis após load/socket (sem depender do spacer flex).
  useEffect(() => {
    if (messagesList.length > 0 && pageNumber === 1) {
      scrollToBottom();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [messagesList.length, ticketId]);
  ...
  // Fallback: envio HTTP devolve a mensagem (nota interna / store) mesmo se o socket atrasar
  useEffect(() => {
    const onLocalMessage = (ev) => {
      const detail = ev?.detail;
      if (!detail?.message) return;
      const msg = detail.message;
      const msgTicketId = msg.ticketId ?? msg.ticket?.id;
      const internalFromRef = ticketInternalIdRef.current;
      const same =
        (msgTicketId != null &&
          currentTicketNumericId.current != null &&
          Number(msgTicketId) === Number(currentTicketNumericId.current)) ||
        (msgTicketId != null &&
          internalFromRef != null &&
          Number(msgTicketId) === Number(internalFromRef)) ||
        (detail.ticketUuid && String(detail.ticketUuid) === String(ticketId));
      if (!same) return;
      dispatch({ type: "ADD_MESSAGE", payload: msg });
      scrollToBottom();
    };
    window.addEventListener("ticket-message-created", onLocalMessage);
    return () => window.removeEventListener("ticket-message-created", onLocalMessage);
  }, [ticketId]);
  ...
  const scrollToBottom = () => {
    ...
    // Duplo rAF: espera layout (bolhas + flex) antes de medir scrollHeight
  ...
  const getCombinedList = () => {
    ...
    const normalizeBubbleBody = (text) =>
      String(text || "")
        .replace(/\u200e/g, "")
        .replace(/\u200c/g, "")
        .replace(/\s+/g, " ")
    ...
    /** FB/IG: remove eco duplicado (verde) da mesma bolha do agente. */
    let visibleMessages = messagesList;
    if (isMetaChannel && Array.isArray(messagesList) && messagesList.length > 1) {
      const preferByBody = new Map();
      messagesList.forEach((m) => {
        if (!m?.fromMe || asBool(m.isPrivate)) return;
        const key = normalizeBubbleBody(m.body);
        if (!key) return;
        const prev = preferByBody.get(key);
        if (!prev) {
          preferByBody.set(key, m);
          return;
        }
        if (asBool(m.fromAgent) && !asBool(prev.fromAgent)) {
          preferByBody.set(key, m);
        }
      });
      ...
      messagesList.forEach((m) => {
        ...
        const key = normalizeBubbleBody(m.body);
        if (!key) return;
        const keep = preferByBody.get(key);
        if (keep && Number(keep.id) !== Number(m.id)) {
          dropIds.add(Number(m.id));
        }
      });
      if (dropIds.size) {
        visibleMessages = messagesList.filter((m) => !dropIds.has(Number(m.id)));
      }
    }
...
>
  {messagesList.length > 0 || logsList.length > 0 ? (
    <>
      {renderMessages()}
      <div
        aria-hidden
        style={{ width: "100%", height: 12, flexShrink: 0 }}
      />
    </>
  ) : !loading ? (
    <div
      style={{
        ...
        fontSize: 13,
        fontWeight: 500,
        color: muiTheme.mode === "dark" ? "rgba(255,255,255,0.72)" : "rgba(0,0,0,0.55)",
      }}
    >
    ...
    <div
      style={{
        ...
        textAlign: "center",
        fontSize: 13,
        color: muiTheme.mode === "dark" ? "rgba(255,255,255,0.55)" : "rgba(0,0,0,0.45)",
      }}
    >
