...
import ContactKanbanSalesPanel from "../ContactKanbanSalesPanel";
  ...
  const history = useHistory();
	...
	useEffect(() => {
		...
		if (open && contact.id) {
			...
			if (needsRefresh && contact?.number) {
				const channel = ticket?.channel || contact?.channel || "whatsapp";
				api
					.get(`/contacts/profile/${encodeURIComponent(String(contact.number).replace(/\D/g, "") || contact.number)}`, {
						params: {
							channel,
		...
		/>
			...
			>
				...
				<div className={classes.contentWrapper}>
					...
					<Avatar
								src={(function(){const u=(contact?.urlPicture||contact?.profilePicUrl)||"";const b=(getBackendUrl&&getBackendUrl());if(!u)return "";if(/^(data:|blob:|https?:\/\/)/i.test(u))return u;return u.startsWith("/")?`${b}${u}`:`${b}/public/${u}`;})()}
								alt={contact.name}
								className={classes.contactAvatar}
								onClick={handleImageClick}
								onError={(e)=>{e.target.onerror=null;e.target.src=((getBackendUrl&&getBackendUrl()))+"/public/app/noimage.png";}}
							>
							...
							<div className={classes.contactCardHeader}>
								...
								/>
								{hasAdsAttribution(adsAttribution) && (
						...
						{loading ? (
							...
							<>
								<TabPanel value={tabValue} index={0} classes={classes}>
									<ContactKanbanSalesPanel contact={contact} ticket={ticket} />
