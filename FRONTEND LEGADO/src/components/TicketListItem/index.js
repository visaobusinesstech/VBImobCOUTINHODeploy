...
const renderChannelBadge = (channel) => {
    ...
    if (ch === "telegram" || ch === "telegram_oficial" || ch === "linkedin") {
        return (
            <SmallAvatar>
                <ConnectionIcon connectionType={ch} width={14} height={14} />
            </SmallAvatar>
        );
    }
    const src = getAvatarChannel(channel);
    if (!src) return null;
    return <SmallAvatar alt={channel} src={src} />;
};
...
const TicketListItem = ({ ticket }) => {
    ...
    useEffect(() => {
        ...
        const local = String(ticket?.contact?.urlPicture || "");
        const pic = String(ticket?.contact?.urlPicture || ticket?.contact?.profilePicUrl || "");
        const stableLocal = local && !/nopicture|noimage/i.test(local) && /\/public\/company|contacts\//i.test(local);
        const needsRefresh =
            !stableLocal &&
            (!pic ||
              /nopicture|noimage/i.test(pic) ||
              (isMeta && /fbcdn|scontent|cdninstagram|fbsbx/i.test(pic)));
    ...
    }, [ticket?.contact?.urlPicture, ticket?.contact?.profilePicUrl, ticket?.contact?.number, ticket?.channel, ticket?.whatsappId]);
            ...
            >
                ...
                <ListItemAvatar>
                    <Badge
                        ...
                        badgeContent={renderChannelBadge(ticket?.channel)}
                    >
                        <ContactAvatar
