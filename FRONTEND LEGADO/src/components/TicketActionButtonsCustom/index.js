...
import AssumeHumanConfirmDialog from "../AssumeHumanConfirmDialog";
import {
  assumeHumanDisabledReason,
  canAssumeHumanOnTicket,
  whatsappHasConnectionAgent,
} from "../../utils/ticketAiAgentPreview";
...
const TicketActionButtonsCustom = ({
  ...
  const [assumeHumanOpen, setAssumeHumanOpen] = useState(false);
  const [assumeHumanLoading, setAssumeHumanLoading] = useState(false);
  ...
  const connectionHasAgent = whatsappHasConnectionAgent(ticket?.whatsapp);
  const canAssumeHuman = canAssumeHumanOnTicket(ticket);
  const assumeHumanReason = assumeHumanDisabledReason(ticket);

  const handleOpenAssumeHuman = () => {
    if (!canAssumeHuman) return;
    handleCloseMenu();
    setAssumeHumanOpen(true);
  };

  const handleConfirmAssumeHuman = async () => {
    if (!user?.id || assumeHumanLoading) return;
    setAssumeHumanLoading(true);
    try {
      await api.put(`/tickets/${ticket.id}`, {
        status: ticket.status === "group" ? "group" : "open",
        userId: user.id,
        isBot: false,
      ...
      toast.success(
        "Você assumiu o atendimento. O agente de IA não responderá mais neste ticket."
      );
      setAssumeHumanOpen(false);
    } catch (err) {
      toastError(err);
    } finally {
      if (isMounted.current) {
        setAssumeHumanLoading(false);
      }
    }
  };
  ...
  return (
    <>
      ...
      <AssumeHumanConfirmDialog
        open={assumeHumanOpen}
        contact={contact || ticket?.contact}
        loading={assumeHumanLoading}
        onCancel={() => {
          if (!assumeHumanLoading) setAssumeHumanOpen(false);
        }}
        onConfirm={handleConfirmAssumeHuman}
      />
      {acceptTicketWithouSelectQueueOpen && (
      ...
      <div className={classes.actionButtons}>
        ...
        >
          {(ticket.status === "open" || ticket.status === "group") && (
            <>
              ...
              >
                ...
                {i18n.t("messagesList.header.buttons.resolve") || "Finalizar"}
              </MenuItem>
              {(ticket.status === "open" || ticket.status === "group") && (
                <Tooltip
                  title={
                    canAssumeHuman
                      ? "Parar o agente de IA neste ticket e assumir o atendimento"
                      : assumeHumanReason
                  }
                  placement="left"
                >
                  <span>
                    <MenuItem
                      disabled={!canAssumeHuman}
                      onClick={handleOpenAssumeHuman}
                      style={
                        !canAssumeHuman
                          ? { opacity: 0.45, color: "#9ca3af" }
                          : undefined
                      }
                    >
                      <ListItemIcon style={{ minWidth: 32 }}>
          ...
          <MenuItem onClick={() => { setAddToListOpen(true); handleCloseMenu(); }}>
            Adicionar à lista
          </MenuItem>
      ...
      <CreateLeadSaleModal
        ...
        hideTicketPreview
        lead={(() => {
          const c = contact || ticket?.contact || null;
          return {
            contactId: c?.id || null,
            ...
            email: c?.email || "",
            contact: c || undefined,
            attribution: c?.attribution || ticket?.attribution || null
          };
        })()}
        onSave={(saved) => {
