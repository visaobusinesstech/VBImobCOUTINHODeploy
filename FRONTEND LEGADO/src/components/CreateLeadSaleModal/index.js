...
import {
  ...
  resolveOriginLabel,
  originChannelIdFromAttribution,
  parseCartLinesFromLead,
  buildLeadDescription,
  stripProductMetaFromDescription,
  extractPriorityFromTags,
  mergePriorityIntoTags
} from "./leadWizardConstants";
import { resolveAttribution } from "../MetaAdsAttributionPanel";
...
const useStyles = makeStyles((theme) => ({
  ...
  createNameSticky: {
    ...
    borderBottom: `1px solid ${theme.palette.type === 'dark' ? 'rgba(255,255,255,0.08)' : '#ECEEF1'}`,
    background: theme.palette.type === 'dark' ? 'rgba(255,255,255,0.02)' : '#FBFBFA'
  },
  splitMenubarRow: {
    flexShrink: 0,
    width: '100%',
...
export default function CreateLeadSaleModal({ open, onClose, lead, onSave, pipelineId, columns, hideTicketPreview = false }) {
  ...
  useEffect(() => {
    ...
    // Rehidrata create-from-ticket quando contact chega depois do open
    const currentId = lead?.id
      ? String(lead.id)
      : `__new__:${lead?.contactId || ""}:${lead?.name || ""}:${lead?.phone || ""}`;
    ...
    const hydrateFromLead = (source) => {
      const attr = resolveAttribution(
        source?.attribution,
        source?.contact?.attribution
      );
      const originFromAds = originChannelIdFromAttribution(attr);
      const originRaw = source?.origin || "";
      const originId =
        resolveOriginChannelId(originRaw) || originFromAds || "";
      const originLabel =
        resolveOriginLabel(originId || originRaw) ||
        originRaw ||
        (originFromAds
          ? resolveOriginLabel(originFromAds)
          : "");
      const cart = parseCartLinesFromLead(source);
      const savedPriority = extractPriorityFromTags(source?.tags) || "Média";
      const firstCurrency = cart.find((l) => l.currency)?.currency;

      setForm({
        name: source?.name || source?.contact?.name || source?.contact?.pushName || "",
        description: stripProductMetaFromDescription(source?.description || ""),
        status: source?.status || "novo",
        value: source?.value || 0,
        companyName: source?.companyName || "",
        contactId: source?.contactId || source?.contact?.id || null,
        responsibleId: source?.responsibleId || null,
        dateStart: source?.date ? String(source.date).slice(0, 10) : today,
        site: source?.site || "",
        origin: originLabel,
        attribution: attr || source?.attribution || null,
        document: source?.document || "",
        birthDate: source?.birthDate ? String(source.birthDate).slice(0, 10) : "",
        address: {
          cep: source?.address?.cep || "",
      ...
      setPhone(source?.phone || source?.contact?.number || "");
      setEmail(source?.email || source?.contact?.email || "");
      setOriginChannel(originId);
      setCartLines(cart);
      setPriority(savedPriority);
      if (firstCurrency) setCurrency(String(firstCurrency).toUpperCase());
      setProductService(cart[0]?.name || "");
      setSelectedPipelineId(source?.pipelineId ?? (pipelineId || null));
      // Criação sem nome: abre direto em Pessoais para o campo obrigatório ficar óbvio
      const initialName =
        source?.name || source?.contact?.name || source?.contact?.pushName || "";
      setActiveStep(!source?.id && !(initialName || "").trim() ? "personal" : null);
      if (source?.contact) setSelectedContact(source.contact);
    };
    ...
    if (lead?.id) {
      ...
      hydrateFromLead(lead || {});
      // Só zera carrinho/prioridade em lead 100% em branco; não apaga prefill do ticket
      const hasPrefill =
        Boolean(lead?.name || lead?.contactId || lead?.phone || lead?.origin) ||
  ...
  }, [lead?.id, lead?.contactId, lead?.name, lead?.phone, open]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (lead?.contactId) {
          ...
          if (!matchedContact) {
            try {
              const { data: byId } = await api.get(`/contacts/${lead.contactId}`);
              if (byId?.id) matchedContact = byId;
            } catch { /* ignore */ }
          }
        } else if (lead?.phone) {
        ...
        batchUpdates(() => {
          ...
          if (matchedContact) {
            ...
            setForm((prev) => {
              ...
              let changed = false;
              if (!prev.contactId) {
                next.contactId = matchedContact.id;
                changed = true;
              }
              const contactName =
                matchedContact.name || matchedContact.pushName || "";
              if (!(prev.name || "").trim() && contactName) {
                next.name = contactName;
                changed = true;
              }
              return changed ? next : prev;
            });
            if (!(phone || "").trim() && matchedContact.number) {
              setPhone(matchedContact.number);
            }
            if (!(email || "").trim() && matchedContact.email) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, lead?.id, lead?.contactId, lead?.phone]);

  useEffect(() => {
    if (!selectedContact) return;
    const newPhone = selectedContact.number || "";
    // Não sobrescreve telefone que o usuário já digitou
    setPhone((prev) => {
      const prevDigits = String(prev || "").replace(/\D/g, "");
      if (prevDigits.length >= 8) return prev;
      return prev === newPhone ? prev : newPhone;
    });
    const contactName = selectedContact.name || selectedContact.pushName || "";
    if (contactName) {
      setForm((prev) => {
        if ((prev.name || "").trim()) return prev;
        return { ...prev, name: contactName, contactId: prev.contactId || selectedContact.id };
      });
    }
  }, [selectedContact]);
  ...
  // Herda atribuição Meta/Google Ads do ticket ou contato vinculado
  useEffect(() => {
    if (!open) return;
    const attr = resolveAttribution(
      ticket?.attribution,
    ...
    setForm((prev) => {
      if (prev.attribution && JSON.stringify(prev.attribution) === JSON.stringify(attr)) {
        return prev;
      }
      return { ...prev, attribution: attr };
    });

    const adsOriginId = originChannelIdFromAttribution(attr);
    if (adsOriginId && !originChannel) {
      setOriginChannel(adsOriginId);
      setForm((prev) => ({
        ...prev,
        attribution: attr,
        origin: resolveOriginLabel(adsOriginId) || prev.origin
      }));
    }
  }, [
    open,
  ...
  useEffect(() => {
    ...
    setForm((prev) => {
      const match = stageOptions.find(
        (opt) => String(opt.value).toLowerCase() === String(prev.status || "").toLowerCase()
      );
      if (match) {
        if (String(match.value) === String(prev.status)) return prev;
        return { ...prev, status: match.value };
      }
      // Em edição, não sobrescreve status desconhecido; em criação, usa a primeira etapa.
  ...
  const handleSubmit = async () => {
    ...
    const trimmedName = (form.name || lead?.name || "").trim();
    if (!trimmedName) {
      toast.error(
        lead?.id
          ? "Informe o nome do lead para salvar."
          : "Informe o nome do lead para criar."
      );
    ...
    try {
      ...
      const originLabel =
        resolveOriginLabel(originChannel || form.origin) ||
        (form.origin || "").trim() ||
        undefined;
      const productsPayload = cartLines.map((l) => ({
        id: l.id,
        name: l.name,
        price: Number(l.price) || 0,
        currency: String(l.currency || currency || "BRL").toUpperCase(),
        qty: Math.max(1, Number(l.qty) || 1)
      }));
      const tagsWithPriority = mergePriorityIntoTags(form.tags, priority);
      const payload = {
        name: trimmedName,
        description: buildLeadDescription(form.description, productsPayload),
        status: form.status,
        ...
        birthDate: form.birthDate && String(form.birthDate).trim() !== "" ? form.birthDate : null,
        address: (() => {
        ...
        contactId: form.contactId || lead?.contactId || null,
        responsibleId:
      ...
      if (lead && lead.id) {
        const payloadWithPipeline = { ...payload, pipelineId: numericPipelineId ?? null };
      ...
      setLoading(false);
      toast.success(lead?.id ? "Lead atualizado com sucesso." : "Lead criado com sucesso.");
      try {
      ...
      // toastError silencia 500/rede — aqui o usuário precisa ver o motivo do save falhar
      if (!status || status >= 500) {
        toast.error(
          typeof apiMsg === "string" && apiMsg && apiMsg !== "Internal server error"
            ? apiMsg
            : "Não foi possível salvar o lead no banco. Tente novamente em instantes."
        );
      } else {
        toastError(err);
      }
