...
export function resolveStageColor(stageKey, stageOptions) {
  const opt = Array.isArray(stageOptions)
    ? stageOptions.find((o) => String(o.value).toLowerCase() === String(stageKey || "").toLowerCase())
    : null;
...
export default function LeadStageNotionTag({
  ...
  const current =
    stageOptions.find((o) => String(o.value).toLowerCase() === String(status || "").toLowerCase()) ||
    null;
  const color = resolveStageColor(current?.value || status, stageOptions);
  ...
  return (
    <div className={classes.row}>
      ...
      {viewOnly ? (
        <NotionTag label={label} color={color} />
      ) : (
        <>
          <div
          ...
          >
            ...
            {stageOptions.map((opt) => (
              <MenuItem
                ...
                selected={String(opt.value).toLowerCase() === String(status || "").toLowerCase()}
                onClick={() => {
