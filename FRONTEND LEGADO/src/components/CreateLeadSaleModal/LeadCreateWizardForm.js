...
import { WIZARD_STEPS, ORIGIN_CHANNELS, PRIORITY_TAG_PREFIX } from "./leadWizardConstants";
import MetaAdsAttributionPanel, {
  hasAdsAttribution
...
export default function LeadCreateWizardForm(props) {
      ...
      <Grid container spacing={gridSpacing} className={formGridClass}>
        <Grid item xs={12} sm={6} className={cellCls}>
          ...
          <TextField
            ...
            inputProps={{ "data-lead-name-input": "form" }}
          />
        </Grid>
        <Grid item xs={12} sm={6} className={cellCls}>
        ...
        <Grid item xs={12} sm={6} data-lead-contact-autocomplete className={cellCls}>
          ...
          <Autocomplete
            ...
            onChange={(_e, val) => {
              ...
              const contactName = val?.name || val?.pushName || "";
              setForm((prev) => ({
                ...prev,
                contactId: val?.id || null,
                name: (prev.name || "").trim() ? prev.name : contactName,
                attribution: val?.attribution || prev.attribution || null
              }));
              if (val?.number) {
                setPhone((prev) => {
  ...
  const sectionProduct = (
    <>
      ...
      <SectionWrap {...sectionWrapProps} title="Pipeline e status">
        <Grid container spacing={gridSpacing} className={formGridClass}>
          <Grid item xs={12} sm={6} className={cellCls}>
            {renderLabel("Pipeline", "pipeline")}
            <FormControl variant="outlined" fullWidth size="small">
              <Select
                value={selectedPipelineId ?? ""}
                onChange={(e) => setSelectedPipelineId(e.target.value === "" ? null : e.target.value)}
                input={<OutlinedInput {...inputPropsBase} />}
                disabled={ro("pipeline")}
              >
                <MenuItem value="">
                  <em>Padrão</em>
                </MenuItem>
          ...
          <Grid item xs={12} sm={6} className={cellCls}>
            {renderLabel("Responsável", "responsible")}
            <FormControl variant="outlined" fullWidth size="small">
              <Select
          ...
          <Grid item xs={12} sm={6} className={cellCls}>
            {renderLabel("Prioridade", "priority")}
            <div className={classes.choiceGrid}>
              {["Baixa", "Média", "Alta", "Crítica"].map((p) => (
                <button
                  ...
                  onClick={() => {
                    if (ro("priority")) return;
                    setPriority(p);
                  }}
                  disabled={ro("priority")}
                >
                  {p}
                </button>
              ))}
          ...
          <Grid item xs={12} sm={6} className={cellCls}>
            ...
            <TextField
              ...
              InputProps={{ ...inputPropsBase, readOnly: ro("site") }}
            />
          </Grid>
        </Grid>
      </SectionWrap>
  ...
  const sectionOrigin = (
    ...
    >
      ...
      <MetaAdsAttributionPanel
        dense
        attribution={form.attribution}
        title="Rastreio de anúncio (Meta / Google)"
      />
      {hasAdsAttribution(form.attribution) && (
        <Box mt={1}>
          <Button
            size="small"
            variant="outlined"
            ...
            onClick={() => setTrackingDetailOpen(true)}
            style={{ textTransform: "none", fontWeight: 600, borderColor: "#1877F2", color: "#1877F2" }}
          >
      ...
      <MetaAdsTrackingDetailModal
        open={trackingDetailOpen}
        onClose={() => setTrackingDetailOpen(false)}
        attribution={form.attribution}
        extras={{
          officialNumber: selectedContact?.number || phone,
          whatsappName: props.ticket?.whatsapp?.name,
          wabaId: props.ticket?.whatsapp?.waba_id,
          phoneNumberId: props.ticket?.whatsapp?.phone_number_id
        }}
      />
    </SectionWrap>
  );
      ...
      <Box mt={2}>
        ...
        <div className={classes.tagInputRow}>
          {Array.isArray(form.tags) &&
            form.tags
              .filter((t) => !String(t).toLowerCase().startsWith(PRIORITY_TAG_PREFIX))
              .map((t, idx) => (
              <Chip
                key={`${t}-${idx}`}
                label={t}
