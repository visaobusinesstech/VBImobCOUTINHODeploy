...
const handleSaveCampaign = async (values) => {
  try {
    ...
    if (values.openTicket === "enabled" && !selectedQueue) {
      toast.error("Selecione uma fila para os tickets da campanha.");
      return;
    }
    ...
    const dataValues = {
      ...
      statusTicket:
        values.openTicket === "enabled" && values.statusTicket === "closed"
          ? "open"
          : values.statusTicket,
    ...
    dataValues.queueId = selectedQueue || null;
    dataValues.userId = selectedUser?.id || null;
    if (dataValues.openTicket === "enabled" && dataValues.statusTicket === "closed") {
      dataValues.statusTicket = "open";
    }
                ...
                <Grid spacing={2} container>
                    ...
                    >
                      ...
                      <Field
                        ...
                        onChange={(e) => {
                          ...
                          setFieldValue("openTicket", next);
                          if (next === "enabled") {
                            setFieldValue(
                              "statusTicket",
                              values.statusTicket === "closed" ? "open" : values.statusTicket
                            );
                          }
                        }}
                      >
                  ...
                  <Grid xs={12} md={6} item>
                    <FormControl
                      ...
                      className={classes.formControl}
                    >
                      <InputLabel id="campaign-queue-selection-label">
                        {i18n.t("transferTicketModal.fieldQueueLabel")}
                      </InputLabel>
                      <Select
                        labelId="campaign-queue-selection-label"
                        id="campaign-queue"
                        value={selectedQueue || ""}
                  ...
                  <Grid xs={12} md={6} item>
                    ...
                    >
                      ...
                      <Field
                        ...
                        onChange={(e) => {
                          const next = e.target.value;
                          setFieldValue("statusTicket", next);
                          if (next === "open" || next === "pending") {
                            setFieldValue("openTicket", "enabled");
                          }
                        }}
                      >
