...
const createMp3Recorder = () =>
  new MicRecorder({
    bitRate: 128,
    sampleRate: 44100
  });

/** Instância por gravação — evita estado sujo do singleton após cancelar/enviar. */
let mp3Recorder = createMp3Recorder();
...
const useStyles = makeStyles((theme) => {
  ...
  mainWrapper: {
    ...
    marginTop: "auto",
    boxShadow: "none",
    // NÃO usar position:fixed no mobile — isso tira o composer do fluxo flex
    // e a lista de mensagens fica coberta/cortada (precisa rolar como WhatsApp).
    position: "relative",
    zIndex: 2,
  },
  mainWrapperEdge: {
...
const MessageInput = ({
  ...
  useEffect(() => {
    if (ticketStatus === "open" || ticketStatus === "group") {
      ...
      setPlaceHolderText("Mensagem interna…");
    } else {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      await ensureTicketConnection();
      if (editing !== null) {
        await postMessageWithConnectionRetry(() =>
          api.post(`/messages/edit/${editing.id}`, message, sendConfig)
        );
      } else {
        const response = await postMessageWithConnectionRetry(() =>
          api.post(`/messages/${ticketId}`, message, sendConfig)
        );
      ...
      if (err?.code === "ECONNABORTED") {
        ...
        /não possui conexão vinculada|exclu[ií]da a conexão/i.test(apiError)
      ) {
        toastError({
          response: { status: 400, data: { error: "ERR_NO_DEF_WAPP_FOUND" } },
        });
  ...
  const handleStartRecording = async () => {
    setLoading(true);
    try {
      // Permissão sem deixar stream órfã (conflita com o MicRecorder em alguns browsers)
      const permissionStream = await navigator.mediaDevices.getUserMedia({
        audio: true
      });
      permissionStream.getTracks().forEach((track) => track.stop());

      mp3Recorder = createMp3Recorder();
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const messages = await listQuickMessages({
          companyId,
          userId: user.id,
          // Respostas rápidas de texto (isOficial false) em todas as conexões,
          // inclusive WhatsApp API Oficial — templates HSM ficam no seletor dedicado.
          isOficial: "false",
          whatsappId: whatsappId || undefined
        });
        if (cancelled || !isMounted.current) return;
        const options = messages
          .filter((m) => !m.isOficial || (m.message && String(m.message).trim()))
          .map((m) => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user.companyId, user.id, ticketChannel, whatsappId]);
  ...
  const isRecoverableWhatsAppLinkError = (err) => {
    const msg = String(err?.response?.data?.error || err?.response?.data?.message || "");
    return (
      /não possui conexão vinculada/i.test(msg) ||
      /exclu[ií]da a conexão/i.test(msg) ||
      msg === "ERR_NO_DEF_WAPP_FOUND" ||
      msg === "ERR_NO_WAPP_FOUND" ||
      msg === "ERR_WAPP_NOT_INITIALIZED" ||
      msg === "ERR_SENDING_WAPP_MSG"
    );
  };

  /** Força o backend a reatar conexão CONNECTED no ticket antes de retry. */
  const ensureTicketConnection = async () => {
    if (!ticketId) return;
    try {
      await api.get(`/tickets/${ticketId}`);
    } catch {
      /* ignore — o retry do envio decide o resultado */
    }
  };

  const postMessageWithConnectionRetry = async (requestFn) => {
    try {
      return await requestFn();
    } catch (err) {
      if (!isRecoverableWhatsAppLinkError(err)) {
        throw err;
      }
      await ensureTicketConnection();
      return await requestFn();
  ...
  const handleUploadAudio = async () => {
    setLoading(true);
    try {
      const [, rawBlob] = await mp3Recorder.stop().getMp3();
      // mic-recorder-to-mp3 sempre gera MP3 — nunca renomear para .ogg (bytes inválidos)
      const blob =
      ...
      if (!blob.size || blob.size < 2000) {
        toast.error("Áudio muito curto. Segure o microfone por pelo menos 1 segundo.");
        return;
      }
      ...
      if (isMounted.current) {
        // Reata conexão órfã antes do envio (silencioso) e retry automático se precisar
        await ensureTicketConnection();
        await postMessageWithConnectionRetry(() =>
          api.post(`/messages/${ticketId}`, formData)
        );
      }
    } catch (err) {
      toastError(err);
  ...
  const handleCancelAudio = async () => {
    try {
      ...
      // cancelar não precisa alertar se o recorder já parou
    } finally {
      mp3Recorder = createMp3Recorder();
      setRecording(false);
    }
  ...
  const renderPendingAlert = () => {
    ...
    return (
      <Box className={classes.pendingAlert}>
        ...
        <span>Somente mensagem interna até aceitar o ticket.</span>
      </Box>
    );
  };
  ...
  const renderComposerChannelIcon = () => {
    ...
    if (ch.includes("instagram")) {
      return <Instagram style={{ fontSize: 16, color: "#E1306C" }} />;
    }
    if (ch.includes("facebook")) {
      return <Facebook style={{ fontSize: 16, color: "#1877F2" }} />;
    }
      ...
      <>
        ...
        >
          ...
          {(replyingMessage && renderReplyingMessage(replyingMessage)) ||
              ...
              {(privateMessageInputVisible || isTicketPending()) && (
                <div className={classes.flexItem}>
                  <div className={classes.messageInputFieldContainer}>
                    ...
                    {renderVariablesBar()}
                    {renderComposerShell(
                      clsx(classes.composerInput, isTicketPending() && classes.composerInputPending),
                      disableOptionForPending(),
                      true
                    )}
                  </div>
                </div>
            ...
            {!isTicketPending() && renderComposerExtrasMenu()}
