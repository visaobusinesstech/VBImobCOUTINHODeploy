...
export const WizardStep1 = ({
  ...
  const isInstagram = String(channel || "").toLowerCase() === "instagram";
  const isFacebook = String(channel || "").toLowerCase() === "facebook";
  return (
    <Box className={ws.step1Root} maxWidth="100%" mx="auto">
      ...
      <Grid container spacing={1}>
        {!isInstagram ? (
          ...
          <Grid item xs={12}>
            <Typography style={{ fontSize: 12, opacity: 0.8, marginBottom: 4 }}>
              Nome da conexão: usa o @ do Instagram automaticamente.
            </Typography>
      ...
      {isInstagram || isFacebook ? (
        <Box className={ws.sectionWrap}>
          <Typography className={ws.sectionCompact}>
            {isInstagram ? "Instagram" : "Facebook"} · Mensagens
          </Typography>
          <FormControlLabel
            ...
            control={
              <Field
                ...
                size="small"
                color="primary"
                name="importInstagramAccountMessages"
                checked={Boolean(values.importInstagramAccountMessages)}
              />
            }
            label={
              <Box>
                <Typography style={{ fontSize: 13, fontWeight: 600 }}>
                  Importar mensagens da conta
                </Typography>
                <Typography
                  style={{ fontSize: 11, color: "inherit", opacity: 0.75 }}
                >
                  Ligado: puxa DMs/conversas existentes. Desligado: só entram
                  mensagens novas depois desta conexão no CRM.
                </Typography>
              </Box>
            }
          />
      ...
      <Box className={ws.segmented} role="tablist">
        <button
          ...
          onClick={() => {
            ...
            if (typeof handleChangeQueue === "function") {
              handleChangeQueue([]);
            }
          }}
