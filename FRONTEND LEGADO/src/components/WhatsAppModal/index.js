...
const WhatsAppModal = ({
  ...
  const initialState = {
    ...
    importInstagramAccountMessages: false,
  };
    ...
    const fetchSession = async () => {
      ...
      try {
        ...
        setWhatsApp({
          ...
          importInstagramAccountMessages: Boolean(
            data.importInstagramAccountMessages
          ),
        });
  ...
  const handleSaveWhatsApp = async (values, options = {}) => {
    ...
    const queuesEnabled = values.queuesEnabled !== false;
    const whatsappData = {
      ...values,
      queuesEnabled,
      ...
      timeAwaitActiveFlowId: timeAwaitActiveFlowId
        ...
        : null,
      // Modo Simples: backend resolve a fila sistema; não enviar seleção de Filas.
      queueIds: queuesEnabled ? selectedQueueIds : [],
  ...
  const validateWizardStep = (step, values) => {
    if (step === STEP.BASIC) {
      const isInstagramChannel =
        String(channel || values.channel || "").toLowerCase() === "instagram";
      if (
        !isInstagramChannel &&
        (!values.name || values.name.trim().length < 2)
      ) {
  ...
  const handleWizardNext = async (values, submitForm, isSubmitting, setFieldValue) => {
    ...
    if (wizardStep === STEP.QUEUES) {
      ...
      try {
        // Garante modo Simples persistido mesmo se o Formik estiver stale.
        const payload =
          values.queuesEnabled === false
            ? { ...values, queuesEnabled: false }
            : values;
        if (payload.queuesEnabled === false && typeof setFieldValue === "function") {
          setFieldValue("queuesEnabled", false);
        }
        const savedId = await handleSaveWhatsApp(payload, {
