...
import { importCalendarEventToActivity } from "../../services/googleWorkspaceService";
...
const EventDetailsModal = ({
  ...
  const handleImportGoogleEvent = async () => {
    ...
    try {
      ...
      const activity = await importCalendarEventToActivity({
        googleEventId: res.id,
        connectionId:
          res.connectionId || googleCalendarMeta?.connectionId,
      });
      toast.success("Evento importado como atividade no calendário.");
      onGoogleEventImported && onGoogleEventImported(activity);
      onClose && onClose();
    } catch (err) {
      toastError(err);
    } finally {
