...
const useStyles = makeStyles((theme) => {
  ...
  const mutedIcon = isDark ? '#d4d4d8' : '#9CA3AF';
  const filterLabelColor = isDark ? 'rgba(255, 255, 255, 0.82)' : '#64748B';
  const placeholderColor = isDark ? '#c4c4cc' : '#6b7280';
...
const ActivitiesStyleLayout = ({
  ...
  /** Trava altura no viewport e deixa só a área de conteúdo com 1 scroll vertical */
  fillViewport = false,
  /** Um único scroll da página (o do layout), sem barra interna duplicada */
  pageScroll = false,
}) => {
  ...
  return (
    <div
      className={[
        classes.root,
        fillViewport && !pageScroll ? classes.rootFillViewport : "",
        rootClassName,
      ]
        .filter(Boolean)
        .join(" ")}
    ...
    >
      ...
      <div
        ...
        className={
          [
            contentEdgeToEdge ? classes.contentEdgeToEdge : classes.content,
            !scrollContent && !pageScroll ? classes.noScroll : "",
            pageScroll ? classes.pageScroll : "",
          ]
            .filter(Boolean)
            .join(" ")
        }
