...
const reducer = (state, action) => {
    ...
    if (action.type === "UPDATE_TICKET_CONTACT") {
        ...
        const next = state.map((t) => {
            if (Number(t.contactId) !== Number(contact.id)) return t;
            changed = true;
            return {
                ...t,
    ...
    if (action.type === "DELETE_TICKET") {
        ...
        const ticketIndex = state.findIndex((t) => Number(t.id) === ticketId);
        if (ticketIndex === -1) return state;
        const next = [...state];
...
const TicketsListCustom = React.memo((props) => {
    ...
    /** Contatos permitidos quando filtro de lista está ativo (socket não pode furar). */
    const allowedListContactIdsRef = useRef(null);
    ...
    // Pai passava `ticketListIds: []` novo a cada render → RESET limpava abas sem refetch.
    const ticketListIdsKey = JSON.stringify(ticketListIds || []);

    useEffect(() => {
        if (Array.isArray(ticketListIds) && ticketListIds.length > 0) {
            const ids = new Set();
            for (const t of ticketsRaw || []) {
                if (t?.contactId != null) ids.add(Number(t.contactId));
            }
            allowedListContactIdsRef.current = ids;
        } else {
            allowedListContactIdsRef.current = null;
        }
    }, [ticketsRaw, ticketListIdsKey, ticketListIds]);
    ...
    useEffect(() => {
        ...
        dispatch({ type: "RESET" });
        setPageNumber(1);
    }, [status, searchParam, dispatch, showAll, tags, users, forceSearch, selectedQueueIds, whatsappIds, statusFilter, sortTickets, searchOnMessages, onlyUnread, dateStart, dateEnd, ticketListIdsKey]);
    ...
    const { tickets, hasMore, loading } = useTickets({
        ...
        ticketListIds: JSON.stringify(ticketListIds || []),
    ...
    useEffect(() => {
        ...
        const shouldUpdateTicket = ticket => {
            const allowedContacts = allowedListContactIdsRef.current;
            if (allowedContacts) {
                const cid =
                    ticket?.contactId != null
                        ? Number(ticket.contactId)
                        : ticket?.contact?.id != null
                          ? Number(ticket.contact.id)
                          : null;
                if (cid == null || !allowedContacts.has(cid)) {
                    return false;
                }
            }
            const sameUser =
            ...
            const queueOk =
                ticketQueueId == null
                    ? showTicketWithoutQueue || noQueueFilter
                    : noQueueFilter ||
                      selectedQueueIds.some((q) => Number(q) === ticketQueueId);
            return (!ticket?.userId || sameUser || showAll) && queueOk;
        }
        ...
        const notBelongsToUserQueues = (ticket) =>
            ticket.queueId &&
            !selectedQueueIds.some((q) => Number(q) === Number(ticket.queueId));
        ...
        const onCompanyTicketTicketsList = (data) => {
            ...
            if (data.action === "update") {
                if (ticketStatusMatchesTab(data.ticket.status, status) && shouldUpdateTicket(data.ticket)) {
                    ...
                    !ticketStatusMatchesTab(data.ticket.status, status) &&
                    !(isMyOpenTicket(data.ticket) || keepOficialOpenTicket(data.ticket))
                ) {
                    // Só remove se saiu desta aba — não apaga ticket open/bot por filtro de fila
                    dispatch({
                        type: "DELETE_TICKET",
        ...
        const onCompanyAppMessageTicketsList = (data) => {
            if (data.action === "create") {
                if (ticketStatusMatchesTab(data.ticket.status, status, data.ticket) && shouldUpdateTicket(data.ticket)) {
                    ...
                    !ticketStatusMatchesTab(data.ticket.status, status, data.ticket) &&
                    !(isMyOpenTicket(data.ticket) || keepOficialOpenTicket(data.ticket))
                ) {
                    dispatch({
                        type: "DELETE_TICKET",
        ...
        const onLocalTicketClosed = (event) => {
            const closedId = event?.detail?.ticketId;
        ...
        window.addEventListener("ticket-closed", onLocalTicketClosed);
        ...
        return () => {
            ...
            window.removeEventListener("ticket-closed", onLocalTicketClosed);
        };

    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [status, showAll, userId, companyId, selectedQueueIds, tags, users, profile, queueIdsStr, sortTickets, showTicketWithoutQueue, socket, ticketListIdsKey]);
