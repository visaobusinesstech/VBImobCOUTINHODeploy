...
export function mergeCrmPlansWithStripeCatalog(stripeProducts) {
  ...
  return CRM_PRICING_PLANS.filter(plan => byKey.has(String(plan.id).toLowerCase())).map(
    plan => {
      ...
      const anualTotalFromStripe = prices.anual?.unitAmount != null ? prices.anual.unitAmount / 100 : null;
      const anual = anualTotalFromStripe != null ? Math.round(anualTotalFromStripe / 12) : plan.prices.anual;
      const annualTotal = anualTotalFromStripe != null ? anualTotalFromStripe : plan.annualTotal;
